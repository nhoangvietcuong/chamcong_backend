...
const maxFileSizeMb = parseInt(process.env.MAX_IMAGE_SIZE_MB || '10', 10);
...
const fileFilter = (req, file, cb) => {
  ...
  if (!allowedMimeTypes.includes(file.mimetype)) {
    return cb(new BadRequestError('Chỉ chấp nhận ảnh định dạng JPEG, PNG, WEBP hoặc video MP4, WEBM, OGG', ERROR_CODE.INVALID_IMAGE_TYPE), false);
  }
  cb(null, true);
};
