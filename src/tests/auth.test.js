...
describe('Authentication & Session Management Integration Tests', () => {
  ...
  test('11. Token cũ vô hiệu', async () => {
    // Already covered in test 10 (Session REPLACED verify)
  ...
  test('18. EMPLOYEE gọi ADMIN API', async () => {
    // Already covered in test 17
    expect(true).toBe(true);
  });

  test('19. MANAGER gọi ADMIN API', async () => {
    // MANAGER calling an admin only endpoint (e.g. creating/registering passkeys is admin only, or similar)
    ...
    // Test route requiring ADMIN role only
    const res = await request(app)
